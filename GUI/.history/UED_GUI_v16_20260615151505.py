...
def NI6251_plot(data=None):
    ...
    global nd_task, samples, scan_cnt
    nd_task = nd.Task()
    # Acquire data if not provided
    if data is None:
        try:
            data = ni6251_data()
        except Exception:
            try:
                ...
                raw = nd_task.read(number_of_samples_per_channel=N_SAMPLES, timeout=10.0)
                data = np.array(raw)
            except Exception as e:
                print("NI6251_plot read error:", e)
                return
    ...
    try:
        ...
        print("NI6251_plot plot error:", e)
"""
...
new_main_window.pushButton_17.clicked.connect(NI6251_test)
