...
def initialize_NI6251():
    ...
    N_SAMPLES = 10_000 #can be whatever, it's not mandatory to be the same as the trigger rate of the delay generator as it's the nr of trigger signal it takes before stopping the acquisition. To aqcuire for 1 sec, it must be = to the rate
    #N_SAMPLES = int(new_main_window.lineEdit_4.text())
    #NI6251_RATE = int(new_main_window.lineEdit_4.text())
    ...
    with nidaqmx.Task() as task:
        try:
            ...
            task.timing.cfg_samp_clk_timing(rate=NI6251_RATE, source=CLOCK_TERMINAL,
...
def ni6251_data():
    ...
    with nidaqmx.Task() as task:
        ...
        data = np.array(task.read(number_of_samples_per_channel=N_SAMPLES, timeout=10.0), dtype=np.double)
