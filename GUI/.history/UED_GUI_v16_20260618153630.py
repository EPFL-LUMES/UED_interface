    ...
    def run(self):
        ...
        while running:
            ...
            if new_main_window.comboBox.currentText() == 'EXTG':
                ...
                ec.setFileWriterConfig('name_pattern', filename)
                if simulator == 0:
                    pulser.channel[2].set_source('TRIG')#DA CAMBIARE
                    #glaz.startMeasurement()
                    nd_task.start()
                    time.sleep(0.1)
                    ec.sendDetectorCommand('arm')
                    pulser.channel[2].set_source('OFF')#DA CAMBIARE
                    data_diode, scan_out = ni6251_data()
                    a = ec.fileWriterFiles()
                    # print(a['value'])
                    # print(filename+'_data_000001.h5')
...
def sort_imgs(hf):
    ...
    data_diode, scan_out = ni6251_data()
    nd_task.stop()
    print('number of shot acquired by the NI6251 : ', scan_out)
...
def ni6251_data():
    ...
    print('print interno:', nd_task.in_stream.total_samp_per_chan_acquired)
