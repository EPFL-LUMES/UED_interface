        ...
        if new_main_window.comboBox.currentText() == 'EXTG':
            ...
            if simulator == 0:
                ...
                while filename+'_data_000001.h5' not in a['value']:
                    ...
                    # print(a['value'])
                    # print(filename+'_data_000001.h5')
                    if np.abs(t0-time.time()) > 10: ### resend trigger after 5 second (if there is not any image to be read)
...
def LTS_acquire_img():
    ...
    if acquisition_mode == "INTS":
        ...
        if simulator == 0:
            ...
            pulser.channel[2].set_source('OFF')
            print('datadiode_1')
            data_diode_1, scan_out = ni6251_data()
...
def ni6251_data():
    ...
    data = np.array(nd_task.read(number_of_samples_per_channel=N_SAMPLES, timeout=10.0), dtype=np.double)
    N_SAMPLES_OUT = nd_task.in_stream.total_samp_per_chan_acquired
    print('INTERNO data from the NI6251 : ', data)
    print('INTERNO nr of samples out : ', N_SAMPLES_OUT)
