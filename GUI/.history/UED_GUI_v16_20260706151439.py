...
DLL_PATH = os.path.join(BASE_DIR, "DLLs", "gladz", "GlazLib.dll")
print("Loading DLL from:", DLL_PATH)
glaz = cdll.LoadLibrary(DLL_PATH)
...
base_measurement_path = r'G:\UED_measurements'
...
def update_roi_diff():
    ...
    if new_main_window.pushButton_14.isChecked() and ROI_chkbx.isChecked():
        ...
        # print('in update img, roi_on:', np.sum(roi_on))
        # print('in update img, roi_off:', np.sum(roi_off))
...
folderpath = r'G:\UED_measurements'
...
##2026JULY: CHIARA ATTEMPT TO FIX ERROR WITH XYZT SCAN, PART1
...
def save_image_h5(filename, array, array_2 = None, data_diode = None):
    ...
    #JULY2026: CHIARA ATTEMPT TO FIX XYZT SCAN, PART2
    dset.attrs['Simstep_positions'] = np.array([
...
def set_quadro_settings():
    ...
    # ec.setDetectorConfig('retrigger_applied',False)  # doesn't exist
    ...
    def run(self):
        ...
        if new_main_window.comboBox.currentText() == 'EXTG':
            ...
            if simulator == 0:
                ...
                data_diode, scan_out = ni6251_data()
                ...
                while filename+'_data_000001.h5' not in a['value']:
                    ...
                    # print(a['value'])
                    # print(filename+'_data_000001.h5')
                    if np.abs(t0-time.time()) > 10: ### resend trigger after 10 second (if there is not any image to be read)
...
def sort_imgs(hf):
    ...
    """
    #Old versione with the gladz, for some reason on 19.6.2026 the NI6251 was inverting them so this version stopped working
    #So we wrote another one, the one below, just exchanging the img_arr and arr_2 in the two if else
    # print(data_diode[0:15])
    # print(scan_out)
    NI6251_plot(data_diode[0:16])
    print('data from the NI6251 : ', data_diode)
    # print(np.mean(data_diode[0::2]), np.mean(data_diode[1::2]))
    if data_diode[1] > data_diode[0]:  # if odd indeces are pump on
        print('pump is on in odd')
        # print('sum data_diode even : ', np.sum(data_diode[::2]))
        # print('sum data_diode odd : ', np.sum(data_diode[::1]))
...
class Scan_Thread(QThread):
    ...
    def run(self):
        ...
        print( 'he step that I need to do : ', np.arange(int(start_pos), int(end_pos), int(stepsize)))
        time.sleep(2)
...
if simulator == 0:
    ...
    index = 5 #june 2026: com15, must be index 5
...
scan_cnt =  int(new_main_window.lineEdit_4.text()) #nr of total triggers
...
nd_task = None
NI6251_DEVICE = "Dev1"
NI6251_AI_CHANNEL = "ai0"
NI6251_CLOCK_TERMINAL = "/Dev1/PFI0"
NI6251_THRESHOLD = 1 #Volt, if one wants to sort by high and low values

#N_SAMPLES = 10_000
#NI6251_RATE = 200_000.0
N_SAMPLES = int(new_main_window.lineEdit_4.text())
#NI6251_RATE = int(new_main_window.lineEdit_4.text())
NI6251_RATE = int(new_main_window.lineEdit_8.text())

def initialize_NI6251():
    global nd_task, N_SAMPLES, NI6251_RATE
    global NI6251_DEVICE, NI6251_AI_CHANNEL, NI6251_CLOCK_TERMINAL
    global NI6251_THRESHOLD
    ...
    if nd_task is not None:
        try:
            print("nd_task already defined")
            nd_task.close()
        except Exception:
            pass

    try:
        nd_task = nidaqmx.Task()
        nd_task.ai_channels.add_ai_voltage_chan(
            f"{NI6251_DEVICE}/{NI6251_AI_CHANNEL}",
            terminal_config=TerminalConfiguration.DIFF,
            min_val=-10.0,
            max_val=10.0,
        )
        nd_task.timing.cfg_samp_clk_timing(
            rate=NI6251_RATE,
            source=NI6251_CLOCK_TERMINAL,
            active_edge=Edge.RISING,
            sample_mode=AcquisitionType.CONTINUOUS,
            samps_per_chan=N_SAMPLES,
        )
        print("NI USB-6251 initialized correctly")
    except nidaqmx.errors.DaqError as e:
        print("DAQmx error:", e)                 # human message
        print("details:", e.args)                # tuple with details
        print("code:", getattr(e, "error_code", None))
...
def set_NI6251():
    ...
    if nd_task is not None:
        print(" ")
        print("NI6251 setted and initialized")
        print("nr of samples that will be acquired by the NI6251: ", N_SAMPLES)
        print("rate used for the acquisition by the NI6251: ", NI6251_RATE)
        print(" ")
...
def ni6251_data():
    global nd_task
    if nd_task is None:
        raise RuntimeError("NI6251 is not initialized. Click the initialize button first.")

    #print("Acquiring data")
    data = np.array(nd_task.read(number_of_samples_per_channel=N_SAMPLES, timeout=60.0), dtype=np.double) 
    N_SAMPLES_OUT = nd_task.in_stream.total_samp_per_chan_acquired
    return data, N_SAMPLES_OUT


niplt_wdg = new_main_window.gladz_plot
niplt = niplt_wdg.plot()


def NI6251_plot(data=None):
    """Plot data from a USB-6251 (uses `ni6251_data()` if available).

    If `data` is not provided, attempts to read from the configured `nd_task`.
    """
    if data is None:
        #print("catching data")
        data, n_samples_out = ni6251_data()
        
    #print("don't need to catch data")
    #data, n_samples_out = ni6251_data()
    
    #print("nr of samples acquired by the NI6251: ", n_samples_out)
    #rate_out = nd_task.timing.samp_clk_rate
    #print("rate used for the acquisition by the NI6251: ", rate_out)
    
    # Plot (mirror gladz_plot behavior)
    """
...
def NI6251_test():
    ...
    DEVICE = "Dev1"
    AI_CHANNEL = "ai0" 
    CLOCK_TERMINAL = "/Dev1/PFI0" # where the trigger is wired 
    ...
    #N_SAMPLES = int(new_main_window.lineEdit_4.text())
    #RATE = int(new_main_window.lineEdit_4.text())
    RATE = 200_000.0
    ...
    with nidaqmx.Task() as task:
        ...
        N_SAMPLES_OUT = task.in_stream.total_samp_per_chan_acquired

    levels = ["HIGH" if v > THRESHOLD else "LOW" for v in data]
    #print(levels)
    #print(data)
    print("nr of samples acquired by the NI6251 in the test run: ", N_SAMPLES_OUT)

    meas_data = np.array(data)

    plt.close('all')
    plt.figure()
    plt.plot(meas_data.flatten(), marker = '+', linestyle ='None')
...
new_main_window.pushButton_7.clicked.connect(initialize_NI6251)
new_main_window.pushButton_9.clicked.connect(set_NI6251)
new_main_window.pushButton_11.clicked.connect(NI6251_plot)
new_main_window.pushButton_17.clicked.connect(NI6251_test) #new button just for test purpose
